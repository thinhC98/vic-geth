...
func (set *unconfirmedBlocks) Insert(index uint64, hash common.Hash, coinbase common.Address) {
	...
	log.Info("[Miner]🔨 mined potential block", "number", index, "hash", hash)
}
...
func (set *unconfirmedBlocks) Shift(height uint64) {
	...
	for set.blocks != nil {
		...
		switch {
		case header == nil:
			log.Warn("[Miner] Failed to retrieve header of mined block", "number", next.index, "hash", next.hash)
		case header.Hash() == next.hash:
			log.Info("[Miner] 🔗 block reached canonical chain", "number", next.index, "hash", next.hash)
		default:
			...
			if included {
				log.Info("[Miner] ⑂ block became an uncle", "number", next.index, "hash", next.hash)
			} else {
				log.Info("[Miner] 😱 block lost", "number", next.index, "hash", next.hash, "creator", next.coinbase)
			}
		}
